...
Context delegate behavior cho current state. Mỗi state biết action nào hợp lệ và transition nào được phép.

## Góc nhìn thực tế

State không nên được dùng chỉ vì tên pattern nghe "xịn". Nó chỉ đáng dùng khi giúp code bớt phụ thuộc sai chỗ, làm thay đổi trong tương lai rẻ hơn, và tạo seam rõ ràng để test.
