...
抽象工厂 的核心是让调用方依赖稳定契约，把真正变化的部分移动到独立对象或协作结构中。

## 实战视角

不要为了“用了设计模式”而使用 抽象工厂。它应该解决真实的变化压力：减少错误依赖、降低未来修改成本，并让测试边界更清晰。

## 真实场景

- TypeScript 项目中的创建型职责正在出现多个变体。
